...
int main(int argc, char **argv) {
  ...
  while (wb_robot_step(TIME_STEP) != -1) {
     ...
     */
     ...
     if (tiempo <= 20){
       if ((pos[0] < sp)&&(pos[2] >= 0)){
        ...
        // double thsteer = atan(rs/rc);
         wb_motor_set_position(steer, thsteer);
         wb_motor_set_velocity(steer, 1.5);
         wb_motor_set_control_pid(steer, 1.5, 1.74, 0.01);
         estado = 1;
       }
     }
     else if (tiempo > 20){
      if ((pos[0] < sp)&&(pos[2] < 0)){
         ...
         wb_motor_set_position(steer, thsteer);
         wb_motor_set_velocity(steer, 1.5);
         //wb_motor_set_control_pid(steer, 0.3, 0.75, 0.18);
         wb_motor_set_control_pid(steer, 0.71, 7, 0.035); 
         estado = 3;
       }
      else if ((pos[0] >= sp)&&(pos[2] < 0)){
         wb_motor_set_torque(longitudinal, newVel);
         ...
         wb_motor_set_velocity(steer, 1.5);
         //wb_motor_set_control_pid(steer, 0.3, 0.75, 0.18);
         wb_motor_set_control_pid(steer, 1.7, 1.5, 0.01); 
         estado = 4;
       }
     }
     j++;
     ...
     printf("W = %frad/s\n", thetap[1]);
     printf("Wsph = %frad/s\n", phip[2]);
     printf("Debug = %f\n", rc);//the*180/3.1416); //thsteer*180/3.1416);
