  ...
  while (wb_robot_step(TIME_STEP) != -1) {
     ...
     */
     ...
     if (pos[0] < sp){
       ...
       /*error = betaref - beta;
       if (error <= 1){
       wb_motor_set_available_torque(longitudinal, 0.1);
       }
       else if (error > 1){
       wb_motor_set_available_torque(longitudinal, 1.5);
       }*/
       //wb_motor_set_position(longitudinal, betaref); 
       //wb_motor_set_velocity(longitudinal, 1.5);
       //wb_motor_set_control_pid(longitudinal, 10, 5, 3);
       //temp1 = sin(alpha);
       //temp2 = r1/rG*temp1;
       //the = asin(temp2) - alpha;
       //thsp = phi + the; 
       //*wb_motor_set_torque(longitudinal, newVel); 
       //wb_motor_set_position(longitudinal, thsp);
       //wb_motor_set_control_pid(longitudinal, 36, 5, 3);
       //wb_motor_set_position(steer, 0.0);
       //wb_motor_set_velocity(steer,0.0);
     }
      ...
      // double thsteer = atan(rs/rc);
       ...
       //wb_motor_set_position(longitudinal, thsp);
       //wb_motor_set_control_pid(longitudinal, 36, 5, 3);
       //thsteer = (1 - 0.01)*(1.5708 - 0.032*thetap[1])+0.01*th2;
     ...
     printf("W = %frad/s\n", thetap[1]);
     printf("Wsph = %frad/s\n", phip[2]);
     printf("Debug = %f\n", spz2);//the*180/3.1416); //thsteer*180/3.1416);
